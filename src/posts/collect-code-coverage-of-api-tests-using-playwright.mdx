...
- **Out-of-the-box support for TypeScript:**
  ...
  If you use Jest or Mocha for tests in a TypeScript project, you have to configure some transform (Babel, `ts-jest`, etc.), or run the test runner with `ts-node`, or compile the test cases before running them (see also [AUTOGENERATE](https://jestjs.io/docs/getting-started#using-typescript))).
- **Support for (ES) module features:**
  ...
  This could for example happen because Jest did not support the `exports` field of a `package.json` (until Jest 28, which was released just a few weeks ago, [AUTOGENERATE](https://jestjs.io/blog/2022/04/25/jest-28#packagejson-exports)).  
...
- **Access to features unique to Playwright:**
  ...
  - "fixtures" allow to prepare resources per test case or worker ([AUTOGENERATE](https://playwright.dev/docs/test-fixtures)).
    This is in my opinion much better than the `beforeEach` / `afterEach` fiddling you have to do with other test runners.
  - "projects" allow to run "the same or different tests in multiple configurations" ([AUTOGENERATE](https://playwright.dev/docs/test-advanced#projects)).
- **Development funded by a big company:**
  ...
  At least it seems like it is getting more traction again in these days (Jest got some releases in the last couple of weeks and got transferred to the OpenJS Foundation, [AUTOGENERATE](https://jestjs.io/blog/2022/05/11/jest-joins-openjs)).
...
- **No use of globals:** Playwright does not use any globals for its test runner.
  ...
  Cypress even has a guide dedicated to this problem ([AUTOGENERATE](https://docs.cypress.io/guides/tooling/typescript-support)).
...
- **Code coverage:**
  ...
  See this commit of the example repository I made: [AUTOGENERATE](https://github.com/pkerschbaum/blog-playwright-for-unit-tests_example/compare/base-setup...code-coverage).
...
- **Fake timers:**
  Use [`sinon`](https://www.npmjs.com/package/sinon), see this commit of the example repository I made: [AUTOGENERATE](https://github.com/pkerschbaum/blog-playwright-for-unit-tests_example/compare/code-coverage...fake-timers).
- **CommonJS/ES module mocking:**
  Jest provides a way to mock CommonJS modules via `jest.mock` (ESM is not supported at the time of writing: [AUTOGENERATE](https://github.com/facebook/jest/issues/10025)); since Playwright Test is executed with Node.js, [`testdouble`](https://github.com/testdouble/testdouble.js) should work (but not tested).
...
- **Snapshot Suffix:**
  ...
  This is an open issue ([AUTOGENERATE](https://github.com/microsoft/playwright/issues/11134)), for now we can disable the suffix by setting up an "auto-fixture" and override `testInfo.snapshotSuffix`:
