...
- **Out-of-the-box support for TypeScript:**
  ...
  If you use Jest or Mocha for tests in a TypeScript project, you have to configure some transform (Babel, `ts-jest`, etc.), or run the test runner with `ts-node`, or compile the test cases before running them (see also [jestjs.io/docs/getting-started#using-typescript](https://jestjs.io/docs/getting-started#using-typescript)).
- **Support for (ES) module features:**
  ...
  This could for example happen because Jest did not support the `exports` field of a `package.json` (until Jest 28, which was released just a few weeks ago, [jestjs.io/blog/2022/04/25/jest-28#packagejson-exports](https://jestjs.io/blog/2022/04/25/jest-28#packagejson-exports)).  
...
- **Access to features unique to Playwright:**
  ...
  - "fixtures" allow to prepare resources per test case or worker ([playwright.dev/docs/test-fixtures](https://playwright.dev/docs/test-fixtures)).
    This is in my opinion much better than the `beforeEach` / `afterEach` fiddling you have to do with other test runners.
  - "projects" allow to run "the same or different tests in multiple configurations" ([playwright.dev/docs/test-advanced#projects](https://playwright.dev/docs/test-advanced#projects)).
- **Development funded by a big company:**
  ...
  At least it seems like it is getting more traction again in these days (Jest got some releases in the last couple of weeks and got transferred to the OpenJS Foundation, [jestjs.io/blog/2022/05/11/jest-joins-openjs](https://jestjs.io/blog/2022/05/11/jest-joins-openjs)).
...
- **No use of globals:** Playwright does not use any globals for its test runner.
  ...
  Cypress even has a guide dedicated to this problem ([docs.cypress.io/guides/tooling/typescript-support](https://docs.cypress.io/guides/tooling/typescript-support)).
...
Fortunately, we can fill those gaps quite easily:

- **Code coverage:**
  Use the command-line interface [`nyc`](https://github.com/istanbuljs/nyc) of Istanbul (a code coverage tool for JavaScript) the same way you would for other TypeScript Node.js projects.
  I have written a dedicated blog post how to set it up for Playwright: TODO LINK
- **Fakes/Spies/Stubs/Mocks:**
  Use [`jest-mock`](https://www.npmjs.com/package/jest-mock) (the same mocking solution shipped with Jest) or [`sinon`](https://www.npmjs.com/package/sinon).
- **Fake timers:**
  Again, use [`sinon`](https://www.npmjs.com/package/sinon).
- **CommonJS/ES module mocking:**
  Jest provides a way to mock CommonJS modules via `jest.mock` (ESM is not supported at the time of writing: [github.com/facebook/jest/issues/10025](https://github.com/facebook/jest/issues/10025)); since Playwright Test is executed with Node.js, [`testdouble`](https://github.com/testdouble/testdouble.js) should work (but not tested).
...
- **Snapshot Suffix:**
  ...
  This is an open issue ([github.com/microsoft/playwright/issues/11134](https://github.com/microsoft/playwright/issues/11134)), for now we can disable the suffix by setting up an "auto-fixture" and override `testInfo.snapshotSuffix`:
