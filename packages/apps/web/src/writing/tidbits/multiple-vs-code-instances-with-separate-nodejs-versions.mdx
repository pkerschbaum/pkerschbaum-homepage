...
So what we want to have is for each project, start an instance of VS Code with the correct Node.js version in use.
...
The tool <FancyAnchor href="https://github.com/nvm-sh/nvm">nvm</FancyAnchor> let's you install multiple versions of Node.js and switch between them as you needed.  
...
But, you can also put the version number in a file `.nvmrc` in the root of your repository and just run `nvm use` without a version number; it will pick up the version defined in the file.  
...
In <FancyAnchor href="https://github.com/microsoft/vscode/issues/1895#issuecomment-290351238">a `microsoft/vscode` GitHub issue</FancyAnchor> I learned that when you start VS Code using the `code` CLI, it will **inherit the environment variables** of the terminal you run that command in. And even if the environment variables of that terminal change, those changes will not propagate to the VS Code instance which was started before.
...
VS Code will use that Node.js version for new integrated terminals instances, pre-commit hooks; just 💫*everywhere*💫.
