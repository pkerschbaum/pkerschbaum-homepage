...
lastUpdatedAtISO: '2022-12-13T22:17:00.000Z'
description: The tsconfig.json configuration I start from for new TypeScript projects.
tags: ['typescript']
---
...
According to <FancyAnchor href="https://web.dev/publish-modern-javascript/">an article from web.dev</FancyAnchor> and its accompanying <FancyAnchor href="https://youtu.be/cLxNdLK--yI">Chrome Developers video</FancyAnchor> from late 2020, `ES2017` is a good choice:
...
But there is one important requirement: a Node.js private mapping **must start with a** `#` and **at least one letter must follow before the slash**.  
...
{/* prettier-ignore */}<FancyAnchor href="https://www.typescriptlang.org/tsconfig#importsNotUsedAsValues">#importsNotUsedAsValues docs</FancyAnchor>

This forces you to use `import type` for import statements if the imported thing is used as a _type_ only (and not used as a _value_).
...
And the following are just my personal preference:
