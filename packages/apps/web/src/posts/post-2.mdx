---
title: Building a Prisma-like API using Type Argument Inference and Conditional Types
published: false
publishedAtISO: '2022-07-05T18:00:00.000Z'
...
/*
  Type of the returned `user` contains the fields of the schema model `User` - not more:
    {
      id: number;
      email: string;
    }
...
To be clear, this is not simple generics going on here; Prisma is actively analyzing the function argument and adapting the shape of the return type based on that.

**The goal of this blog post is to understand the main idea about how that works.**  
...
We will only focus on changing the return type depending on whether `include` is given or not.
...
A good start is to type it such that `orgUnit` is optional in the return type:
...
We will now incrementally improve on that draft.

## Using Type Argument Inference and Conditional Types to improve the typing
...
```typescript
function findUniqueOrThrow<T extends FindArgs>(args: T): Promise<FindResult>;
```
...
type FindResult<T extends FindArgs> = { // <-- `FindResult` now also takes `T` as does `findUniqueOrThrow`
  id: number;
  email: string;
  orgUnit?: {
    id: number;
...
We can achieve this goal with a typescript feature called _<FancyAnchor href="https://www.typescriptlang.org/docs/handbook/2/conditional-types.html">Conditional Types</FancyAnchor>_.
...
Let's change `FindResult` to this:

```typescript
type FindResult<T extends UserFindUniqueArgs> = {
  id: number;
  email: string;
...
An intersection type just allows you to add something to an existing type.
...
The conditional type `T extends { include: { orgUnit: true } }` does exactly that. Remember, `T` captures _the actual type_ of the function call, and the conditional type checks if `T` is assignable to `{ include: { orgUnit: true } }`.  
Essentially, we are asking "was the function invoked with an argument containing `{ include: { orgUnit: true } }`?" If the answer is "yes", we add the property `orgUnit` to the result type.  
Otherwise, we add an empty object - which does not change anything of the result type.

...and that's it! `findUniqueOrThrow` will now react to the actual argument given, just like Prisma Client does!

## One last thing: a glance at the types of Prisma Client
...
Turns out that `UserGetPayload` - the equivalent of our `FindResult` type - is also far more complicated than our type...
...
Still, you can see some similarities to our solution. It also inspects `include` and, if it is present and contains `orgUnit`, goes on with `OrgUnitGetPayload`, which has the fields for the `OrgUnit` model.

The main concept remains: capture the actual argument in a generic variable, and use conditional types to adapt based on that variable!
