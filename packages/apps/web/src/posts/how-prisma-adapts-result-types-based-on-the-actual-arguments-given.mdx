...
shortenedURL: how-prisma-adapts-result-types-based-on-the-actual-arguments-given
...
Intersection types all to add some stuff to existing types.
...
I remember situations in which the type of something was `undefined` or `null` and I thought: I _know for sure_ that this thing is defined here, why does TypeScript not know that?  
...
And the concept we learned in this blog post - the combination of generics and conditional types - can be one tool in our toolbelt to improve the typing of APIs, such that we do not have to resort to some of those nasty quick-fixes!
...
The biggest difference is that they do everything step by step:
